...
开始生成任何测试代码前，必须先完整检查 `pom.xml`，不能跳过，也不能在用户提醒后才回头补。

强制检查项：

- 是否已有 `spring-boot-starter-test`
- 是否已有 `org.jacoco:org.jacoco.agent`
- `maven-surefire-plugin` 是否显式配置了：
  - `skipTests=false`
  - `forkCount=0`
  - `jacoco-agent.destfile`
- 是否已有 `jacoco-maven-plugin`
  - `instrument`
  - `restore-instrumented-classes`
  - `report`
  - `outputDirectory=${project.build.directory}/jacoco-ut`

执行规则：

1. 如果 `pom.xml` 缺少上述任一必要依赖或插件，必须先补齐 `pom.xml`，再开始生成测试类。
2. 如果项目已有同类配置，必须先复用现有配置，不要重复添加冲突插件。
3. 如果补过 `pom.xml`，必须重新执行测试验证，不允许只生成测试文件就结束。
4. 生成测试后，必须检查：
   - `target/jacoco.exec` 是否存在
   - `target/jacoco-ut/index.html` 是否存在
   - 目标类 HTML 是否不是 `0%`

禁止：

- 先写测试，跑通后再回头补 `pom.xml`
- 没检查 `pom.xml` 就默认项目已有 JaCoCo 或 Surefire 配置
- 用户明确要求按 skill 执行时，仍然遗漏依赖或插件补齐步骤
## 总体策略
...
每次处理这类项目时，必须按下面顺序执行，不能调换：

1. 读取并检查 `pom.xml`
2. 补齐必要测试依赖和插件
3. 再生成或修复测试类
4. 执行 `mvn test` 或指定测试类验证
5. 检查 JaCoCo 产物是否生成
6. 打开 HTML 或 XML/CSV 核对目标类/目标方法覆盖率
7. 若未达到要求，继续补测，而不是直接结束

如果本次任务包含“按某个 skill 生成单测”，那么“检查并补齐 `pom.xml`”属于必须自动执行的前置步骤，不需要等待用户再次提醒。
## 最终默认建议
