...
**⚠️ 核心原则：只清理用户明确指定的 Input/Output，绝对禁止清理用户未提供的！**
...
**重要**：
- ✅ 只处理用户列表中的 Input/Output
- ❌ 禁止清理用户列表之外的任何 Input/Output
- ❌ 禁止删除包含用户未指定常量的接口
- ❌ 禁止在类级别清理影响其他方法的代码
...
**⚠️ 关键检查：只有在用户指定的所有 Output 方法都已清理，且类中没有其他未清理的 Output 方法时，才执行类级别清理！**

**检查时机**：
- ✅ 用户指定的所有 Output 方法都已清理完成
- ✅ 类中没有其他使用 `outputInterface` 或 `RocketMQGrayUtils` 的方法
...
**⚠️ 关键检查：只有在用户指定的所有 Input 方法都已清理，且类中没有其他未清理的 Input 方法时，才执行类级别清理！**

**检查时机**：
- ✅ 用户指定的所有 Input 方法都已清理完成
- ✅ 类中没有其他使用 `@StreamListener` 的方法
...
**⚠️ 核心原则：绝对禁止删除包含用户未指定常量的接口！**

**重要原则**：
- ✅ 只有当接口中**所有**常量都已被用户指定且已清理时，才能删除接口
- ❌ 如果接口中还包含任何用户未指定的常量，**绝对不能删除**
- ✅ 删除接口前，必须逐个检查接口内的所有常量定义
...
2. **列出接口中所有常量**（不仅仅是用户指定的）
3. **逐个检查每个常量**：
   - 是否在用户指定的 Input 列表中？
   - 是否已被清理？
4. **判断逻辑**：
   - ✅ 如果**所有**常量都在用户列表中且都已清理 → 接口为空，可以删除
   - ❌ 如果**任何**常量不在用户列表中 → 接口不为空，不能删除
   - ❌ 如果**任何**常量还未清理 → 接口不为空，不能删除
...
**⚠️ 核心原则：只有当整个项目中不再有任何地方调用 `RocketMQGrayUtils` 时，才应删除该工具类。**
...
- ✅ **只清理用户明确指定的 Input/Output**
- ❌ **绝对禁止清理用户未提供的 Input/Output**
- ❌ **绝对禁止删除包含未指定常量的接口**
- ❌ **绝对禁止在类级别清理影响其他方法的代码**

**错误示例**：
```
用户指定: xxx-input, yyy-output
错误行为: 删除整个 InputInterface（即使其中还有 zzz-input）
正确行为: 只清理 xxx-input 和 yyy-output，保留 InputInterface（因为还有 zzz-input）
```
...
**检查示例**：
```java
// InputInterface.java 内容
public interface InputInterface {
    String XXX_INPUT = "xxx-input";      // 用户指定了
    String YYY_INPUT = "yyy-input";      // 用户指定了
    String ZZZ_INPUT = "zzz-input";      // 用户未指定！
}

// 用户列表: xxx-input, yyy-output
// ❌ 不能删除 InputInterface，因为还有 zzz-input
```
...
**⚠️ 在清理类级别代码（@EnableBinding、字段、构造函数）之前，必须检查**：
- ✅ 用户指定的**所有**方法都已清理
- ✅ 类中没有其他使用相关依赖的方法

**不能清理类级别代码的情况**：
- ❌ 类中还有其他未清理的方法（即使用户未指定）
- ❌ 其他方法还在使用 `outputInterface` 或 `@StreamListener`
...
public class MessageProducer {
    private final OutputInterface outputInterface;  // 是否能删除？
    ...
    public void sendYyy(YyyDTO data) {
        // 用户未指定，还在使用灰度切换！
        RocketMQGrayUtils.gray(...);
    }
}

// ❌ 不能删除 outputInterface 字段，因为 sendYyy() 还在使用
```
...
**核心原则检查**：
- [ ] 只清理了用户指定的 Input/Output
- [ ] 没有清理用户未指定的 Input/Output
- [ ] 没有删除包含未指定常量的接口

**方法级别清理**：
- [ ] 所有用户指定的 Output 方法都已清理（或确认不需要清理）
- [ ] 所有用户指定的 Input 方法都已清理（或确认不需要清理）

**类级别清理**（仅在满足条件时执行）：
- [ ] MessageProducer 类的 @EnableBinding 注解已移除（仅当所有用户指定的方法已清理且无其他方法使用）
- [ ] MessageProducer 类的 outputInterface 字段已移除（仅当无其他方法使用）
- [ ] MessageReceiver 类的 @StreamListener 注解已移除（仅用户指定的方法）
- [ ] MessageReceiver 类的 @EnableBinding 注解已移除（仅当所有用户指定的方法已清理且无其他方法使用）
...
**接口清理**（仅在满足条件时执行）：
- [ ] 已检查接口中所有常量（不仅仅是用户指定的）
- [ ] 只有所有常量都在用户列表中且已清理时，才删除接口
- [ ] 接口引用已清理
