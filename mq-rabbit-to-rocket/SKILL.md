...
5. **检查启动类 Apollo 配置**
   ```
   搜索 @EnableApolloConfig 注解
   ```
   检查是否已包含 `rocketmq.yml` namespace。如果未包含，需要在迁移步骤中添加。
...
4. 检查启动类 `@EnableApolloConfig` 是否包含 `rocketmq.yml`
...
**重要：这是迁移的第一步，必须先完成此步骤才能让 RocketMQ 配置生效。**

在启动类的 `@EnableApolloConfig` 注解中添加 `rocketmq.yml` namespace：

### 修改前

```java
@EnableApolloConfig(value = {"sqs.common", "dbConnection.yml", "application", "common.bootstrap", "redis.yml", "hystrix.yml", "stream.yml", "third.yml"})
...
```java
@EnableApolloConfig(value = {"sqs.common", "dbConnection.yml", "application", "common.bootstrap", "redis.yml", "hystrix.yml", "stream.yml", "third.yml", "rocketmq.yml"})
public class XxxApplication {
    public static void main(String[] args) {
        SpringApplication.run(XxxApplication.class, args);
    }
...
1. **Namespace 顺序**：`rocketmq.yml` 通常放在最后，确保其他配置优先加载。
...
### 4.2 修改后（同时支持 RocketMQ 和 RabbitMQ）
...
`@EnableApolloConfig` 必须包含 `rocketmq.yml` namespace，否则 RocketMQ 配置无法加载：
...
**问题5：RocketMQ 配置不生效**
- 原因：启动类 `@EnableApolloConfig` 未包含 `rocketmq.yml`
- 解决：在 `@EnableApolloConfig` 的 value 数组中添加 `"rocketmq.yml"`
...
- [ ] 启动类 `@EnableApolloConfig` 已添加 `rocketmq.yml` namespace
...
- [ ] 延迟消息类型已正确转换
- [ ] 消息体类型保持一致
- [ ] 未排除 `RabbitAutoConfiguration`（灰度期间）
...
```java
@EnableApolloConfig(value = {"sqs.common", "dbConnection.yml", "application", "common.bootstrap", "redis.yml", "hystrix.yml", "stream.yml", "third.yml", "rocketmq.yml"})
public class OnlineThirdApplication {
    public static void main(String[] args) {
        SpringApplication.run(OnlineThirdApplication.class, args);
    }
}
```
