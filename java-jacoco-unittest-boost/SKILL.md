...
- 补充单测前必须先确认项目现有测试可正常运行；若现有测试失败，先按 `java-unittest` skill 的测试修复流程修正基线测试，修正后再继续本专项。
...
4. 修复基线测试时允许修改已有测试代码，但仅限让原有测试恢复正常运行；仍严禁修改业务代码。修复后必须重跑测试确认通过。
5. 基线测试恢复正常后，记录基线检查命令、结果和修复摘要到 `runs/<project-key>/jacoco-unittest-progress.md`，再继续 JaCoCo 配置解析和逐类补测。
6. 若基线测试无法修复，停止本专项，输出失败原因和待人工处理项，不开始新增覆盖率补测。
...
重试时只针对未覆盖方法、分支和异常路径补充测试。不得为了覆盖率修改生产代码，不得删除已有测试，不得降低断言质量。若某一轮补充后测试代码爆红或 Maven 编译失败，不继续对该类重试，直接记录为 `不通过` 并处理下一个类。
...
记录基线测试检查命令与结果、最终测试命令、报告 XML/HTML 路径、整体覆盖率、临时 JaCoCo excludes 是否已加入/是否已还原，以及仍需人工决策的问题。
...
- [ ] 补测前已运行基线测试；若基线失败，已先按 `java-unittest` 修复并重跑通过。
...
- [ ] 遇到测试代码爆红或编译失败的类，已记录为 `不通过` 并跳过到下一个类。
