...
- 失败的本轮新增测试不保留草稿，直接回退本轮新增内容，只在 `.claude/jacoco-unittest-agent-log.md` 记录原因。
...
解析 `target/site/jacoco/jacoco.xml`、`target/jacoco-ut/jacoco.xml` 或项目 POM 中指定的 JaCoCo XML 报告路径。
...
根据 JaCoCo XML 报告生成目标清单：

1. 读取报告中的 class/sourcefile 覆盖率数据。
...
- 生成测试导致编译失败且无法在本轮新增文件内修复时，记录为 `不通过`；失败的本轮新增测试不保留草稿，直接回退本轮新增内容，只在 `.claude/jacoco-unittest-agent-log.md` 记录原因，然后继续下一个目标类。
