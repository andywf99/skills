...
- 补充单测前必须先确认项目现有测试可正常运行；若现有测试失败，先按 `java-unittest` skill 的测试修复流程修正基线测试，最多修复 5 次，修正后再继续本专项。
- 禁止通过空跑测试、无断言测试、mock 被测类自身、降低覆盖率阈值、扩大 excludes 排除真实业务类、反射调用无业务意义私有方法等方式刷覆盖率。
- 进度文件属于本 Skill 的运行产物，必须写在 `java-jacoco-unittest-boost/runs/` 下，不写入业务项目目录。
- 进度文件必须实时更新：目标清单生成、开始处理类、补充测试完成、测试代码爆红/编译失败、覆盖率验证完成、类达标或跳过时都要立即落盘。
- 测试代码不通过时，严禁把原有测试文件或新增测试文件移动到业务项目的备份目录、改后缀或移出 `src/test/java`。应仅还原本轮新增/追加的失败测试改动，保证项目测试目录保持可编译状态，并在进度文件中记录失败原因。

## 执行流程

### 0. 基线测试检查

1. 在补充任何新单测前，先运行项目现有测试，确认基线测试可正常编译并通过。单模块项目优先运行 `mvn test`；多模块项目运行实际业务 module 的 `mvn -pl <module> -am test`。
2. 若项目测试耗时过长，可先运行目标 module 或现有测试集合，但必须覆盖当前补测范围的测试编译链路。
3. 若现有测试失败，不进入目标类补测；先读取并遵循 `java-unittest` skill，优先使用其 `parse_test_result.py` 解析失败原因，按分类修复编译错误、依赖缺失、Mock 注入失败、断言不匹配等问题。
4. 基线测试最多修复 5 次。每次修复后都必须重跑测试，并将 attempt、错误摘要、修复动作、重跑结果实时记录到 `runs/<project-key>/jacoco-unittest-progress.md`。
5. 修复基线测试时允许修改已有测试代码，但仅限让原有测试恢复正常运行；仍严禁修改业务代码。
6. 基线测试恢复正常后，记录基线检查命令、结果和修复摘要到 `runs/<project-key>/jacoco-unittest-progress.md`，再继续 JaCoCo 配置解析和逐类补测。
7. 若连续 5 次修复后基线测试仍不通过，立即停止本专项，不开始新增覆盖率补测；告知用户：“当前模型不行，使用已有的单测不能修复”，并输出失败命令、最后一次错误摘要、已尝试修复动作和待人工处理项。
...
6. 将目标清单和处理进度记录到本 Skill 目录下的 `runs/<project-key>/jacoco-unittest-progress.md`；若目录不存在则创建。`project-key` 使用项目目录名加路径哈希，避免不同项目重名覆盖。长任务恢复时先读取该文件，继续处理第一个未完成目标类。
...
状态枚举只允许使用：`通过`、`不通过`。处理中的中间节点、编译失败、覆盖率不足、跳过原因等写入“说明”列，不扩展状态枚举。
...
7. 开始处理当前类、补充测试代码写入、验证完成等节点都要实时更新进度文件；状态只写 `通过` 或 `不通过`，当前处理阶段写入“说明”列。
8. 如果新增或补充的单测代码出现明显爆红、编译失败、依赖无法解析、JUnit/Mockito API 不兼容、IDE 无法识别符号等问题，状态记录为 `不通过`，在说明中写明“编译失败/测试代码爆红”、错误摘要、测试类路径、attempt 次数，立即跳过当前类并进入下一个目标类。
9. 跳过前必须清理本轮失败测试改动：新建的失败测试文件可删除；追加到已有测试类的失败代码只回退本轮新增的 import、字段、方法和辅助代码，严禁移动或删除原有测试文件。若需要保留失败草稿用于排查，只能复制到本 Skill 的 `runs/<project-key>/failed-tests/` 下，不得写入业务项目备份目录。
...
重试时只针对未覆盖方法、分支和异常路径补充测试。不得为了覆盖率修改生产代码，不得删除已有测试，不得降低断言质量。若某一轮补充后测试代码爆红或 Maven 编译失败，不继续对该类重试，直接记录为 `不通过`，还原本轮失败测试改动后处理下一个类。

每个关键节点都更新 `runs/<project-key>/jacoco-unittest-progress.md`，不得等到一批类结束后再补写。每处理完 5 个类或阶段性暂停时，向用户汇报一次：
...
需要生成每个类的具体测试代码，或基线测试运行失败需要修复时，读取并遵循 `java-unittest` skill，尤其是：
...
如果因测试代码爆红或编译失败跳过，也写入未达标记录：

```text
类全路径 | 测试类路径 | 状态=不通过 | attempt | 错误摘要 | 后续建议
```

### 最终 JaCoCo 报告

记录基线测试检查命令与结果、基线修复 attempt 次数、最终测试命令、报告 XML/HTML 路径、整体覆盖率、临时 JaCoCo excludes 是否已加入/是否已还原，以及仍需人工决策的问题。若基线测试 5 次修复后仍失败，最终报告只输出基线失败结论，不输出 JaCoCo 补测结果。
...
- [ ] 补测前已运行基线测试；若基线失败，已先按 `java-unittest` 修复，最多 5 次，重跑通过后才继续。
- [ ] 若基线测试连续 5 次修复仍不通过，已停止运行并告知用户“当前模型不行，使用已有的单测不能修复”。
- [ ] 如临时补充 JaCoCo excludes，已记录 POM 变更状态；默认已还原，或已记录用户明确要求保留。
- [ ] 多模块项目已定位实际业务 module，并使用正确的 Maven `-pl/-am` 命令。
- [ ] 已维护本 Skill 目录下的 `runs/<project-key>/jacoco-unittest-progress.md` 进度文件，可断点续跑，且未把进度文件写入业务项目目录。
- [ ] 进度文件已在每个关键节点实时更新，不是任务结束后补写。
- [ ] 遇到测试代码爆红或编译失败的类，已记录为 `不通过`，已还原本轮失败测试改动，并跳过到下一个类。
- [ ] 未将失败测试文件移动到业务项目备份目录；如保留失败草稿，仅保存到本 Skill 的 `runs/<project-key>/failed-tests/`。
