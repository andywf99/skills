...
## 0. PowerMock 灰度开关测试规范

当被测方法涉及灰度开关时，**必须**遵循以下规范：

### 测试框架

- 使用 `PowerMockRunner` 和 `@PrepareForTest` 注解
- 通过 `Whitebox.invokeMethod` 调用私有方法
- 使用 `spy` 创建被测类实例，`mock` 创建依赖对象
- 静态方法 Mock：使用 `PowerMockito.mockStatic()`
...
public class XxxServiceTest {
    ...
    public void setUp() throws Exception {
        ...
        service = spy(new XxxService());
        Whitebox.setInternalState(service, "dependencyService", dependencyService);
    }
}
```
...
**必须**覆盖以下场景，每个场景独立一个测试方法：

**场景一：灰度开关关闭**
```java
@Test
public void test_xxxMethod_grayDisabled_callOldLogic() throws Exception {
    // Mock 灰度开关关闭
    mockStatic(GrayUtils.class);
    when(GrayUtils.isGray(any(GraySwitch.class))).thenReturn(false);

    // 执行测试
    Whitebox.invokeMethod(service, "xxxMethod", params);
...
```java
@Test
public void test_xxxMethod_grayEnabled_executeNewLogic() throws Exception {
    // Mock 灰度开关开启
    mockStatic(GrayUtils.class);
    when(GrayUtils.isGray(any(GraySwitch.class))).thenReturn(true);
...
对于涉及字段更新的逻辑，**必须**覆盖以下场景：

**场景一：字段为空时更新**
```java
@Test
public void test_xxxMethod_fieldsEmpty_fieldsUpdatedFromSource() throws Exception {
    // Mock 灰度开关开启
    mockStatic(GrayUtils.class);
    when(GrayUtils.isGray(any(GraySwitch.class))).thenReturn(true);
    ...
    Source source = createSource("value1", "value2");

    // 执行测试
    Whitebox.invokeMethod(service, "xxxMethod", record, source);

    // 验证字段被更新
    assertThat(record.getField1()).isEqualTo("value1");
    assertThat(record.getField2()).isEqualTo("value2");
}
```
...
```java
@Test
public void test_xxxMethod_recordFieldsNotEmpty_fieldsNotUpdated() throws Exception {
    // Mock 灰度开关开启
    mockStatic(GrayUtils.class);
    when(GrayUtils.isGray(any(GraySwitch.class))).thenReturn(true);
    ...
    // 准备测试数据（字段已有值）
    Record record = new Record();
    record.setField1("existing_value1");
    record.setField2("existing_value2");

    Source source = createSource("new_value1", "new_value2");

    // 执行测试
    Whitebox.invokeMethod(service, "xxxMethod", record, source);

    // 验证字段未更新
    assertThat(record.getField1()).isEqualTo("existing_value1");
    assertThat(record.getField2()).isEqualTo("existing_value2");
}
```

### 边界值测试规则

对于数值型字段，**必须**覆盖 0、null 等特殊值：

**场景：字段为 0 或 null 时更新**
```java
@Test
public void test_xxxMethod_fieldZeroOrNull_updatedFromSource() throws Exception {
    // Mock 灰度开关开启
    mockStatic(GrayUtils.class);
    when(GrayUtils.isGray(any(GraySwitch.class))).thenReturn(true);
    ...
    Whitebox.invokeMethod(service, "xxxMethod", record2, source);

    // 验证字段被更新（因为原值为null）
    assertThat(record2.getId()).isEqualTo(1L);
}
```

### 参数为空测试规则

**必须**覆盖参数为 null、空列表、空字符串等场景：

**场景：参数为 null 时不执行操作**
```java
@Test
public void test_xxxMethod_paramNull_noAction() throws Exception {
    // Mock 灰度开关开启
    mockStatic(GrayUtils.class);
    when(GrayUtils.isGray(any(GraySwitch.class))).thenReturn(true);
    ...
    // 执行测试
    Whitebox.invokeMethod(service, "xxxMethod", record, null);

    // 验证无操作
    assertThat(record.getField1()).isNull();
    assertThat(record.getField2()).isNull();
}
```
 ...
 * 创建测试对象
 */
private Source createSource(String field1, String field2) {
...
- [ ] 使用 `PowerMockRunner` 和 `@PrepareForTest({GrayUtils.class})`
- [ ] 使用 `spy` 创建被测类，`mock` 创建依赖
- [ ] 使用 `Whitebox.invokeMethod` 调用私有方法
- [ ] 覆盖灰度开关关闭场景（验证旧逻辑）
- [ ] 覆盖灰度开关开启场景（验证新逻辑）
- [ ] 字段为空时覆盖更新逻辑
- [ ] 字段不为空时覆盖不更新逻辑
- [ ] 数值型字段覆盖 0、null 等特殊值
- [ ] 参数为 null、空列表、空字符串各有独立测试方法
- [ ] 所有测试方法都有中文注释说明「测试场景 + 覆盖代码行 + 验证点」
...
- 如果项目中有 PowerMock 且需要 mock 静态方法，使用 `PowerMockRunner` + `@PrepareForTest` + `PowerMockito.mockStatic()`；如果只有普通 mock，使用 `MockitoExtension.class`。
