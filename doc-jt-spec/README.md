...
输出 Review 摘要后，逐个提问阻塞问题：一次只问一个问题，每个问题必须调用 Claude Code 的 `AskUserQuestion` 工具，呈现类似 superpowers 的可点击选项控件，提供 2-4 个业务选项并标明推荐选项；自定义内容通过工具自动提供的 Other/其他输入框承接。提问时不能在普通文本里写“选项：1...2...3...”，应直接用工具让用户点击。用户回答当前问题后，再继续下一个问题或创建 OpenSpec change 和 `proposal.md`。
...
一致性检查 + 疑问收口，只问阻塞问题；阻塞问题一次只问一个，并调用 Claude Code 的 `AskUserQuestion` 工具提供 2-4 个业务选项和推荐选项；不得用普通文本编号选项替代工具调用。自定义内容通过工具自动提供的 Other/其他输入框承接，非阻塞问题写入待确认事项。
...
| 一问一答 | 阻塞问题逐个提问，不一次抛出大量问题；每个问题调用 `AskUserQuestion` 工具提供 2-4 个业务选项并标明推荐项，禁止用普通文本编号选项替代工具调用 |
