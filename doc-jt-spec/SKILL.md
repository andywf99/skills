...
- 先探索上下文，再提问。
- 一次只问一个澄清问题，不要把多个问题合并成清单一次性抛出。
- 每个澄清问题和阶段确认都必须调用 Claude Code 的 `AskUserQuestion` 工具提问，呈现类似 superpowers 的可点击选项。
- 提问轮次中禁止在普通文本里输出“选项：”“1.”“2.”“3.”“4.”“5.”等编号选项；阶段摘要可以先用普通文本输出，但真正需要用户选择时，最后一步必须是 `AskUserQuestion` 工具调用。
- 每个 `AskUserQuestion` 问题提供 2-4 个业务选项，并说明推荐选项、适用场景和影响；自定义内容使用工具自动提供的 Other/其他输入框承接，不要手写“其他/自定义输入”选项。
- 如果当前环境无法调用 `AskUserQuestion` 工具，必须明确说明“当前环境不支持交互式选项控件”，然后暂停等待用户输入；不要退化成普通文本编号选项。
...
3. 在聊天中输出 Review 摘要：
   ...
   - 是否阻塞进入 Research
4. 对阻塞问题逐个提问，一次只问一个；每次提问必须调用 Claude Code 的 `AskUserQuestion` 工具，提供 2-4 个业务选项，标明推荐选项，并说明各选项对范围、风险或后续设计的影响。
5. 提问时不要先在普通文本里写“选项：”或编号列表；应直接调用 `AskUserQuestion`，让用户点击选项。
6. 自定义内容通过 `AskUserQuestion` 工具自动提供的 Other/其他输入框承接，不要把“其他/自定义输入”写成普通文本编号选项。
7. 等用户回答当前问题后，再根据回答决定是否继续问下一个阻塞问题；不要在同一轮消息里列出全部待确认问题。
8. 用户确认 Review 通过后，才创建或完善 OpenSpec `proposal.md`。
...
- 一次只问一个问题，不要一次抛出大量问题。
- 每个问题都必须调用 Claude Code 的 `AskUserQuestion` 工具，提供 2-4 个业务选项，标明推荐选项，并说明选择后对最终 TRD 的影响；自定义内容通过工具自动提供的 Other/其他输入框承接。
- 最终确认提问不得输出普通文本编号选项；如果需要展示背景，先展示摘要，再立即调用 `AskUserQuestion` 完成选择。
...
当需要用户确认阻塞问题或阶段是否通过时，必须把最终提问放进 `AskUserQuestion` 工具调用中。可以在工具调用前输出阶段摘要，但不能输出普通文本编号选项。正确示例：

```text
AskUserQuestion({
  questions: [{
    question: "是否确认 Review 通过并进入 Research 阶段？",
    header: "Review确认",
    options: [
      { label: "通过（推荐）", description: "当前需求边界足够进入代码证据核验。" },
      { label: "先补充规则", description: "需要先补充业务规则，再进入 Research。" },
      { label: "暂缓进入", description: "当前信息不足，先暂停后续设计。" }
    ],
    multiSelect: false
  }]
})
...
- 询问用户时必须一次只问一个问题，不要一次性输出问题清单。
- 每个问题必须调用 Claude Code 的 `AskUserQuestion` 工具提问，提供 2-4 个业务选项；第一个选项应为推荐选项并标注“推荐”，同时说明为什么推荐。
- 自定义内容通过 `AskUserQuestion` 工具自动提供的 Other/其他输入框承接；不要把“其他/自定义输入”写成普通文本编号选项。
- 不允许用普通文本输出“选项：1...2...3...”替代工具调用；如果无法调用工具，暂停并说明环境不支持交互式选项控件。
- 可选项要能直接帮助用户决策，例如“按 PRD 原文处理 / 按现有系统口径兼容 / 暂列待确认”，不要只给“是/否”这类缺少业务含义的选项。
- 用户回答当前问题后，再继续下一个问题或进入下一阶段。
